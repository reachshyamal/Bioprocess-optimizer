...
rsm1 <- rsm(Yield~FO(Nano,Superdex)+TWI(Nano,Superdex)+PQ(Superdex),data=do)
summary(rsm1)

rsm3 <- rsm(Yield~TWI(Nano,Superdex),data=do)
summary(rsm3)

rsm4 <- rsm(Yield~FO(Sephacel),data=do)
summary(rsm4)

rsm5 <- rsm(Yield~FO(HA),data=do)
summary(rsm5)

rsm6 <- rsm(Yield~FO(Sepharose),data=do)
summary(rsm6)

rsm7 <- rsm(Yield~FO(Supernetant),data=do)
summary(rsm7)

write.csv(tidy(summary(rsm1)),"rsm1.csv")

############################################################### 

d1 <- d[,c(1:19,26,42,56,58,61)]
colnames(d1)
...
plot(d1$Supernetant,d1$Sepharose)

# Sepharose = f( Supernetant , Sepharose.Load )
rsm8 <- rsm(Sepharose~FO(Supernetant,Sepharose.Load)+TWI(Supernetant,Sepharose.Load),data=d1)
summary(rsm8)

# Sepharose = f( TFF1 parameters , Sepharose parameters )
#TFF1 parameters = DV,Cassette.Area,Flux,TMP,pH,Conductivity
rsm9 <- rsm(Sepharose~FO(DV,Flux,TMP)+TWI(DV,Flux)+PQ(DV,Flux,TMP),data=d1)
summary(rsm9)
rsm10 <- rsm(Sepharose~FO(Conductivity,pH)+
                      TWI(Conductivity,pH)+
                      PQ(pH),data=d1)
summary(rsm10)
...
rsm11 <- rsm(Sepharose~FO(Binding.Capacity,Flow.rate,Max.Pressure)+
                      TWI(Binding.Capacity,Flow.rate,Max.Pressure)+
                      PQ(Binding.Capacity,Max.Pressure),data=d1)
summary(rsm11)
rsm12 <- rsm(Sepharose~FO(Sepharose.Load.pH,Sepharose.Load.Conductivity,Max.OD)+
                       TWI(formula = ~ Sepharose.Load.pH*Sepharose.Load.Conductivity+Sepharose.Load.pH*Max.OD)+
                       PQ(Max.OD),data=d1)
summary(rsm12)
...
rsm14 <- rsm(HA~FO(DV,Cassette.Area,Flux)+TWI(formula = ~ DV*Cassette.Area+Flux*Cassette.Area),data=d2)
summary(rsm14)
rsm15 <- rsm(HA~FO(Conductivity,pH,TMP)+
                TWI(formula = ~ Conductivity*TMP+pH*TMP)+
                PQ(Conductivity,pH,TMP),data=d2)
summary(rsm15)
...
rsm16 <- rsm(HA~FO(Column.Volume,Binding.Capacity,Flow.Rate,Max.OD)+
                TWI(formula = ~ Flow.Rate*Binding.Capacity+Max.OD*Binding.Capacity+Column.Volume*Flow.Rate+Flow.Rate*Max.OD)+
                PQ(Column.Volume,Binding.Capacity,Max.OD),data=d2)
#summary(update(rsm16, . ~ . - 1))
summary(rsm16)
rsm17 <- rsm(HA~FO(Load.pH,Load.conductivity,Max.Pressure)+
                TWI(Load.pH,Load.conductivity)+
                PQ(Load.pH,Load.conductivity),data=d2)
summary(rsm17)
...
rsm22 <- rsm(Sephacel~FO(Sephacel.Load.pH,Sephacel.Load.conductivity,Max.Pressure)+
                      TWI(formula = ~ Sephacel.Load.pH*Sephacel.Load.conductivity+Sephacel.Load.conductivity*Max.Pressure)+
                      PQ(Max.Pressure),data=d3)
summary(rsm22)
...
rsm24 <- rsm(Superdex~FO(DV,Cassette.Area,Flux)+PQ(DV,Cassette.Area,Flux),data=d4)
summary(rsm24)
rsm25 <- rsm(Superdex~FO(Conductivity,pH,TMP)+
               PQ(Conductivity,pH),data=d4)
summary(rsm25)
...
rsm26 <- rsm(Superdex~FO(Column.Volume,Flow.rate)+
                      TWI(Column.Volume,Flow.rate)+
                      PQ(Flow.rate),data=d4)
#summary(update(rsm26, . ~ . - 1))
summary(rsm26)
rsm27 <- rsm(Superdex~FO(Superdex.Load.pH,Superdex.Load.conductivity)+
                      TWI(Superdex.Load.pH,Superdex.Load.conductivity)+
                      PQ(Superdex.Load.pH),data=d4)
summary(rsm27)
